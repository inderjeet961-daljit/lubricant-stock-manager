...
export const packFinishedGoods = async (product_name: string, pack_size: string, quantity: number, cartons: number = 0) => {
  const response = await api.post('/manager/pack-finished-goods', { product_name, pack_size, quantity, cartons });
  return response.data;
};
